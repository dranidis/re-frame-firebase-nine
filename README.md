...
* [`src/re_frame_firebase_9/`](src/re_frame_firebase_9/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/re_frame_firebase_9/core.cljs): contains the SPA entry point, `init`
* [`test/re_frame_firebase_9/`](test/re_frame_firebase_9/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `re-frame-firebase-nine` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/re_frame_firebase_9/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns re-frame-firebase-nine.example
  (:require [re-frame-firebase-nine.config :as config])
